...
implemented mostly in Python providing a rich set of APIs.%abundant collection of APIs for state-of-the-art machine learning techniques in python.
...
% Each table shows the averaged classification accuracy (in the last column only for intra-building case). The classification accuracy is also shown for each experiment, as we vary the percentage of the complete training set (denoted as X\%). Each data subset is repeated 1/X times (where X is the percentage) and the average accuracy is calculated. The accuracy of the baseline approach is shown in parentheses.
...
to bootstrap the classification process. In Table~\ref{table:rice}-~\ref{table:sdh_x}, %,~\ref{table:sdh},~\ref{table:rice_x},~\ref{table:sdh_x}, 
