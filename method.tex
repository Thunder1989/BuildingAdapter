...
We refer to these different classifiers as {\it base} models\footnote{We use the term ``model'' and ``classifier'' {\it interchangably} in this paper.} and will combine them for classification on the new building.
...
If the neighborhs of $x$ on both graphs significant overlap, then $M_i$ will be assigned a larger weight.
...
The use of weighted average decision for $x$ among base classifiers is reasonble when at least some of these classifiers perform well on $x$. However, the similarity $s(G_{M_i}, G_C|x)$ for $M_i$ is expected to be small wehn the predictions of $M_i$ around $x$ conflict with its true local structure. 
